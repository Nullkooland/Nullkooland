...
![新出的三部曲虫豸版，狗都不玩好吗！](https://www.youtube.com/watch?v=TTB6eEHCAko)
...
好友们都有自己的输出，我很羡慕他们，羡慕他们能静下心来写一点，画一点，说一点东西，把自己投入到什么里面，留下一些思绪的痕迹。我曾经也有这样的产出热情，不过最近找不到了。希望在有了自己的小天地之后能够找回那种感觉，至少得对得起自己搭建它所付出的精力。~~笑死，基础设施搭好了根本不写文章！~~
...
没错，这里就是*努克* ~~（某友人：我不管，我就要叫他*崆植*！）~~ 的奇妙世界。但是说不定哪天会钻进来一只老虎。**【更新】2021.11.22**：现在晚上会有*那个老虎*出没！~~（可是垃圾 Windows 并不能在晚上自动切换成 dark mode，所以他要么根本不出来要么持续营业！）~~

## 撒子技术？

基于 [Blazor WebAssembly](http://blazor.net) 技术，差不多就是浏览器加载了基于 `WASM` 的 `.NET` 运行时然后在上面跑的客户端应用。博客都是用 `Markdown` 写的，在网页应用运行时直接加载 `*.md` 文件并通过 [Markdig](https://github.com/xoofx/markdig) 解析并动态渲染成各种组件。代码高亮着色使用了 [Prism](https://prismjs.com)，数学公式渲染使用了 [KaTeX](https://katex.org)，评论系统是基于 GitHub issues 的 [Utterances](https://utteranc.es/)。

目前使用了 [GitHub Actions](https://github.com/actions) 自动部署成静态页面并 host 在 [GitHub Pages](https://pages.github.com) 上。

### 为什么不选择已有的社交媒体平台呢？

1. 不能支持 `Markdown`，$\TeX\ \text{Math}$ 等功能，过于无能。
...
**Let's get started!** ![Laughing Moonbear](images/moonbear.png)
